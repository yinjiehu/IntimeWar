...
namespace IntimeWar
{
	public class SkillPrefabCollection : MonoBehaviour
	{
		static SkillPrefabCollection _instance;
		public static SkillPrefabCollection Get()
		{
			if (_instance != null)
				return _instance;

#if UNITY_EDITOR
			var prefab = Resources.Load<SkillPrefabCollection>("SkillPrefabCollection");
            //var prefab = UnityEditor.AssetDatabase.LoadAssetAtPath<GameObject>("Assets/_Prefabs/Data/AttachmentPrefabs");
#else
			var prefab = Resources.Load<SkillPrefabCollection>("SkillPrefabCollection");
#endif
            _instance = Instantiate(prefab).GetComponent<SkillPrefabCollection>();
