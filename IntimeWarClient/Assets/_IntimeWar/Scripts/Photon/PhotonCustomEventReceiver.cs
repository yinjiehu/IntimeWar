...
namespace IntimeWar
{
    public class PhotonCustomEventReceiver : MonoBehaviour
    {
        ...
        Dictionary<string, Delegate> _registedTypedDelegate = new Dictionary<string, Delegate>();
        Dictionary<string, Action<object>> _registedDirectDelegate = new Dictionary<string, Action<object>>();
                    ...
                    case (byte)PhotonServerEvCodeEnum.CustomEvent:
                        {
                            ...
                            Delegate d1;
                            Action<object> d2;
                            if (_registedTypedDelegate.TryGetValue(evName, out d1))
                            {
                                var type = Type.GetType(evName);
                                d1.DynamicInvoke(JsonUtil.Deserialize((string)data[1], type));
                            }
                            else if (_registedDirectDelegate.TryGetValue(evName, out d2))
                            {
                                if (data.Length == 1)
                                    d2(null);
                                else
