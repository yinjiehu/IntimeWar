...
namespace MechSquad.View
{
	public class HudHp : MonoBehaviour
	{
		...
		public void SetUnitAndStartFollow(BattleUnit unit, Transform follow)
		{
			_observerUnit = unit;
			_body = unit.Body;
			_armor = unit.GetAbility<MechArmor>();

			_previousBodyHp = _body.CurrentHp;
			_targetBodyHpAmount = _previousBodyHp / _body.MaxHp;

			_armorHpBack.gameObject.SetActive(_armor != null);
			if (_armor != null)
			{
				_previousArmorHp = _armor.CurrentHp;
				_targetArmorHpAmount = _previousArmorHp / _armor.MaxHp;
			}
		...
		void UpdateArmorHp()
		{
			...
			var currentHp = _armor.CurrentHp;
			if (_previousArmorHp != currentHp)
			{
				var maxHp = _armor.MaxHp;
				_previousArmorHp = currentHp;
				_targetArmorHpAmount = currentHp / maxHp;
				//Show();
			}

			UpdateHpFillAmount(_armorHpFront, _armorHpBack, _targetArmorHpAmount);
		}
