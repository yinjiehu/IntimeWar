...
namespace MechSquad
{
	public static class Util
	{
		...
		public static Battle.BodyMaterialTypeEnum GetGroundMaterial(Vector3 position)
		{
			position.y = 100;
			var hits = Physics.RaycastAll(position, Vector3.down, 300, LayerMask.GetMask("Surface_Dirt", "Surface_Water"));
			for (var i = 0; i < hits.Length; i++)
			{
				var hit = hits[i];
				if (hit.collider.gameObject.layer == LayerMask.NameToLayer("Surface_Water"))
				{
					return Battle.BodyMaterialTypeEnum.Water;
				}
			}
			return Battle.BodyMaterialTypeEnum.Dirt;
		}
